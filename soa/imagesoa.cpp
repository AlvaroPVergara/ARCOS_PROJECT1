...
void execute_function(const std::filesystem::path& filePath, std::filesystem::path path_out_dir, int(*function)(BmpSOA&)){
    ...
    if (bmp.Export(path_out_dir / filePath.filename()) < 0)
    {
        exit(-1);
    }
}
